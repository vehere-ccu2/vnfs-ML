...
object InputOutputDataHandler {
  ...
  /**
    ...
    */
  def getInputDataFrame(sqlContext: SQLContext, sparkContext: SparkContext, inputPath: String, logger: Logger): Option[DataFrame] = {
    try {
	val srcfile = new java.io.File(inputPath)
	if (srcfile.exists) {
	    logger.info("Loading data from: " + srcfile.toString())
	}else {
		logger.info("File Not Found: " + srcfile.toString())
	}
      //Some(sqlContext.read.json(srcfile.toString()))
      Some(sqlContext.read.parquet(srcfile.toString()))
    } catch {
