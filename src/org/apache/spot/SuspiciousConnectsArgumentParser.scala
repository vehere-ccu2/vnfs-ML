...
object SuspiciousConnectsArgumentParser {
  ...
  case class SuspiciousConnectsConfig(analysis: String = "",
                                      ...
                                      ldaMaxiterations: Int = 50,
                                      ldaAlpha: Double = 1.02,
                                      ldaBeta: Double = 1.001,
                                      esnodes: String = "localhost",
                                      esport : String = "9200")
  ...
  val parser: scopt.OptionParser[SuspiciousConnectsConfig] = new scopt.OptionParser[SuspiciousConnectsConfig]("LDA") {
    ...
    opt[String]("input").required().valueName("<local file>").  //hdfs path
    ...
    opt[String]("scored").required().valueName("<local file>"). //hdfs path
    ...
    opt[Double]("ldabeta").optional().valueName("float64").
      ...
      opt[String]("esport").required().valueName("String").
      action((x, c) => c.copy(esport = x)).
      text("elasticsearch port number")

      opt[String]("esnode").required().valueName("string").
      action((x, c) => c.copy(esnodes = x)).
