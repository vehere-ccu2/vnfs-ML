...
object DNSSuspiciousConnectsModel {
  ...
  def trainNewModel(sparkContext: SparkContext,
    ...
    val totalRecords = selectedRecords.unionAll(DNSFeedback.loadFeedbackDF(sparkContext,
      sqlContext,
      config.feedbackFile,
      config.duplicationFactor,logger))
  ...
  logger.info("timeCuts")
    val timeCuts =
      Quantiles.computeDeciles(totalRecords
        .select(UnixTimestamp)
        .rdd
        .flatMap({ case Row(unixTimeStamp: String) =>
          Try {
            unixTimeStamp.toDouble
          } match {
            case Failure(_) => Seq()
            case Success(unixTimeStamp) => Seq(unixTimeStamp)
          }
        }))
        
    logger.info("frameLengthCuts")

    val frameLengthCuts =
      Quantiles.computeDeciles(totalRecords
        .select(FrameLength)
        .rdd
        .flatMap({ case Row(frameLength: String) =>
          Try {
            frameLength.toDouble
          } match {
            case Failure(_) => Seq()
    ...
    logger.info("domainStatsRecords")
    ...
    logger.info("Starting dnswordCreation.......................................")
